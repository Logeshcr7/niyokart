...
export const TopDealsSection: React.FC<TopDealsSectionProps> = ({
  ...
  // Grab deal phones or fallback to the first 5 phones
  const dealPhones = phones.filter((p) => p.category === 'deal' || p.discountPercent).slice(0, 5);
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 border-b border-slate-200 pb-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            Sizzling discounts on the highest rated phones this week
          </p>
        </div>
          ...
          return (
            ...
            >
              ...
              {phone.discountPercent && (
                <div className="absolute left-3.5 top-3.5 z-10 rounded-md bg-amber-500 px-2 py-0.5 text-[11px] font-black uppercase text-white shadow-sm">
                  {phone.discountPercent}% OFF
                </div>
              )}

              {/* Compare Quick Action (Top Right) */}
              <button
                onClick={(e) => {
                  e.stopPropagation();
              ...
              <div
                ...
                className="relative mt-3 flex h-44 w-full cursor-pointer items-center justify-center overflow-hidden rounded-xl bg-slate-50 p-2"
              >
                <img
                  src={phone.image}
                  alt={phone.name}
              ...
              <div className="mt-3 flex-1 flex flex-col">
                ...
                <div className="mt-1.5 flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] text-slate-400">
                    ({phone.ratingCount ? phone.ratingCount.toLocaleString('en-IN') : '1,200'})
                  </span>
                </div>
                ...
                {/* Action Buttons: Buy Now & Compare */}
